...
const MaterialBlock = ({ name, color, volume, selectedMaterial, setSelectedMaterial }) => {
  ...
  if (name === selectedMaterial) {
    return (
      <button onClick={e => setSelectedMaterial(name)} className="flex items-center p-3 border-b-2 bg-active border-border">
    ...
    return (
      <button onClick={e => setSelectedMaterial(name)} className="flex items-center p-3 border-b-2 border-border">
        {/* block representing the color */}
        <div className={`rounded-full w-[2.4rem] h-[2.4rem] bg-[${matColor}] mr-4`}>
        </div>
        {/* block representing the name and the volume of the material */}
